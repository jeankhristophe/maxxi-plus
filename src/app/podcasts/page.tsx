        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted">
              Annuaire francophone — {podcasts.length} podcasts
            </p>
          </div>
        </div>
      ...
      {viewMode === "grid" ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filtered.map((p, i) => (
            <div
              key={p.id}
              className={`animate-slide-up opacity-0 stagger-${Math.min(i + 1, 8)}`}
            >
        ...
        <div className="space-y-2">
          {filtered.map((p, i) => (
            ...
            >
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[10px] bg-elevated text-muted">
                {p.category}
              </span>
              <span className="text-xs text-subtle">{p.episodeCount} ép.</span>
            </a>
          ))}
        </div>
      )}
